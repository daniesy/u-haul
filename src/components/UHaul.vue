...
const generateInitialPositions = (i = 0, totalHeight = 0) => {
    ...
    setTimeout(() => {
        const itemHeight = itemHeights.get(props.items[i].id) || props.itemHeight;
...
const wrapperHeight = computed(() => props.items.length * props.itemHeight);

let itemHeights = new Map();
...
const calculateStartIndex = scrollTop => {
    ...
    while (startIndex < props.items.length && accumulatedHeight < scrollTop) {
        accumulatedHeight += itemHeights.get(props.items[startIndex].id) || props.itemHeight; // default height
        startIndex++;
    }
...
const calculatePosition = (index) => {
    ...
    for (let i = 0; i < index; i++) {
        top += itemHeights.get(props.items[i].id) || props.itemHeight;
    }
    return top;
}
...
const handleScroll = e => {
    ...
    const afterValues = props.items.length - startIndex < props.extra ? props.items.length - startIndex : props.extra;
    for (let i = 0; i < afterValues; i++) {
        newPositions.push(startIndex + visible.value + i);
    }

    const missingNewPositions = newPositions.filter(i => !positions.value.find(({itemIndex}) => itemIndex === i));
    ...
    if (positions.value.length < newPositions.length) {
        for (let i = 0; i < newPositions.length; i++) {
            ...
            positions.value.push({itemIndex: newPositions[i], top: positions.value[positions.value.length - 1].top});
        }
...
const setItemRef = el => {
    ...
    itemHeights.set(itemId, el.clientHeight);
}
...
<div class="wrapper" @scroll="handleScroll" ref="wrapper">
    <div class="wrapper-height" :style="`height: ${wrapperHeight}px`"></div>
    <component :is="itemTag" v-for="({itemIndex, top}, index) in positions" :key="index"
        :ref="setItemRef"
